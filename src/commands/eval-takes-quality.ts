...
export async function runEvalTakesQuality(engine: BrainEngine, args: string[]): Promise<void> {
  // Self-configure the AI gateway (mirrors cli.ts:connectEngine). Route a
  // file-backed config through the canonical fold so explicit provider keys
  // such as minimax_api_key reach the recipe environment.

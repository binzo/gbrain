...
export interface EmbeddingTouchpoint {
  ...
  /**
   * Maximum number of input items per embedding request. Some providers
   * publish both an aggregate token cap and an item-count cap; when set, the
   * gateway pre-splits batches so each request satisfies both.
   */

...
describe('recipe: dashscope', () => {
  ...
  test('embedding touchpoint supports only text-embedding-v4', () => {
    const r = getRecipe('dashscope')!;
    expect(r.touchpoints.embedding).toBeDefined();
    expect(r.touchpoints.embedding!.models).toEqual(['text-embedding-v4']);
    expect(r.touchpoints.embedding!.default_dims).toBe(1024);
    expect(r.touchpoints.embedding!.model_dims).toBeUndefined();
    expect(r.touchpoints.embedding!.dims_options).toEqual([64, 128, 256, 512, 768, 1024, 1536, 2048]);
    // 2048 is valid for the provider; pgvector falls back to exact scan above
    // HNSW's 2000-dim ceiling.
    expect(r.touchpoints.embedding!.dims_options).toContain(2048);
  });
  ...
  test('dimsProviderOptions threads dimensions only for text-embedding-v4', async () => {
    // DashScope text-embedding-v4 is a Matryoshka model.
    ...
    expect(dimsProviderOptions('openai-compatible', 'text-embedding-v3', 768))
      .toBeUndefined();
    expect(dimsProviderOptions('openai-compatible', 'text-embedding-v2', 1536))
      .toBeUndefined();
  });
});
